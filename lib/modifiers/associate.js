function onStat({ stat, addStat, utils }) {
  const { getNamespacedName, skills, stats, statuses } = utils;
  ...
  if (isSkillType(stat.type) && stat.statFields.associate) {
    ...
    stat.statFields[SHOUT_NAMES.SKILL_PROPERTIES] += `${getNamespacedName(statusName)},${chance},${turns};`;

    stat.statFields.associate.Status_CONSUME[STATUS_CONSUME_NAMES.STATS_ID] = `${getNamespacedName(potionName)}`;

    if (weaponName) {
      stat.statFields.associate.Potion[POTION_NAMES.BONUS_WEAPON] = `${getNamespacedName(weaponName)}`;      
    }
