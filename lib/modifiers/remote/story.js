...
function getCoreStory() {
  ...
  KBSECTION
PROC
  PROC_GENERATED_ASC_REMOTE_DAMAGE_SINGLE((CHARACTERGUID)_Source, (CHARACTERGUID)_Target, (STRING)_Status)
AND
...
PROC
  PROC_GENERATED_ASC_REMOTE_DAMAGE_AE((CHARACTERGUID)_Source, (CHARACTERGUID)_Target, (STRING)_Status)
AND
...
AND
  StringContains(_Status, "${statNames.unformattedRemoteStatusPrefix}", 1)
THEN
  PROC_GENERATED_ASC_REMOTE_DAMAGE_SINGLE(_Source, _Target, _Status);
  PROC_GENERATED_ASC_REMOTE_DAMAGE_AE(_Source, _Target, _Status);
