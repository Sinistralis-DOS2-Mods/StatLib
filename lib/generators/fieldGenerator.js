...
const { POTION_TYPES } = require('../const/potionTypes');

const skillFiles = Object.values(SKILL_TYPES);
const statusFiles = Object.values(STATUS_TYPES);
const potionFiles = Object.values(POTION_TYPES);
...
async function printDefinition(fieldType) {
  ...
  case 'statusFields':
    ...
    break;
  case 'potionFields':
    fieldsDeclare = 'POTION_FIELD';
    directory = './lib/definitions/potionFields.js';
    break;
  }
...
async function generate() {
  ...
  for (let potionFile of potionFiles) {
    await loadCategory(potionFile);
  }

  await printDefinition('potionFields');
}
