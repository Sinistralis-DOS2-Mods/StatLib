...
async function run() {
  ...
  statModifiers.forEach(statModifier => {
    if (statModifier.onLoad) {
      statModifier.onLoad({addStat, utils: { getStatName, enums, skills, stats, statuses } });      
    }
  });
  ...
  */
  while (statQueue.length > 0) {
    ...
    statModifiers.forEach(statModifier => {
      stat = statModifier.onStat({ stat, addStat, utils: { getStatName, enums, skills, stats, statuses } });
    });
  ...
  statModifiers.forEach(statModifier => {
    if (statModifier.getScript) {
      scripts[getStatName(statModifier.name)] = statModifier.getScript();      
    }

    if (statModifier.getStory) {
      stories[getStatName(statModifier.name)] = statModifier.getStory();      
    }
  });
