...
function addStat(type, item) {
  if (type === 'stat') {
    statQueue.push(item);
  } else {
    throw new Error(`Attempted to queue unknown stat of type ${type} and item ${JSON.stringify(item)}.`);
  }
}
  ...
  */
  while (statQueue.length > 0) {
    ...
    statModifiers.forEach(statModifier => {
      stat = statModifier({ stat, addStat, utils: { namespace, enums, skills, stats, statuses } });
    });
