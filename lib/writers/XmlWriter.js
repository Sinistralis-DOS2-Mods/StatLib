...
const { getStatFile, parseXml, fileToXml, writeFile, ensureDirectoryExistence } = require('../util');
...
async function save(data) {
  ...
  const finalFile = getStatFile(data.dataType, modStatsPath);

  ensureDirectoryExistence(finalFile);
  await writeFile(finalFile, dataToWrite);
}
